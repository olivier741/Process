...
//            logger.info("ADD message in the queue :"+ process_req);
    ...
    public static void loadFeatures(int sleep_duration, BlockingQueue<Process_Request> reg_queue) {
        Process_Register.sleep_duration = sleep_duration;
        Process_Register.reg_queue = reg_queue;
        Process_Register.app_conf = commonConfig.getApp_conf();
        Process_Register.processthead_reg = app_conf.getProcess_thread_reg();
        Process_Register.sleep_duration = processthead_reg.getSleep_duration();
