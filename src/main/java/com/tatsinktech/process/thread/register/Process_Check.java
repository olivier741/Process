            ...
            if (process_mo != null) {
                ...
                if (!StringUtils.isBlank(product_code)) {
                    ...
                    switch (useproduct) {
                        case 0:
                            if (oldReg == null) {
                                process_mo.setNotificationCode("CHECK-PRODUCT-NOT-REGISTER-" + product_code);
                                mo_his_desc = "CHECK-PRODUCT-NOT-REGISTER-" + product_code;
                            } else {
                                int status = oldReg.getStatus();
                                if (status == 1) {
                                    process_mo.setNotificationCode("CHECK-PRODUCT-ACTIVE-ACTIVE-" + product_code);  // CUSTOMER IS ACTIVE ON THAT PRODUCT
                                    mo_his_desc = "CHECK-PRODUCT-ACTIVE-ACTIVE-" + product_code;
                                }
                                if (status == 0) {
                                    process_mo.setNotificationCode("CHECK-PRODUCT-ACTIVE-CANCEL-" + product_code);  //CUSTOMER IS CANCEL ON THAT PRODUCT
                                    mo_his_desc = "CHECK-PRODUCT-ACTIVE-CANCEL-" + product_code;
                                }
                                if (status == 2) {
                                    process_mo.setNotificationCode("CHECK-PRODUCT-ACTIVE-PENDING-" + product_code); // CUSTOMER IS EXPIRE ON THAT PRODUCT (waiting to charge) 
                                    mo_his_desc = "CHECK-PRODUCT-ACTIVE-PENDING-" + product_code;
                                }
                                if (status == -1) {
                                    process_mo.setNotificationCode("CHECK-PRODUCT-CANCEL-CANCEL-" + product_code); // CUSTOMER IS BLOC IN OPERATOR SIDE
                                    mo_his_desc = "CHECK-PRODUCT-CANCEL-CANCEL-" + product_code;
                                }
                        ...
                        case 2:
                        case 3:
                            process_mo.setNotificationCode("CHECK-PRODUCT-WRONG-TIME-" + product_code);
                            mo_his_desc = "CHECK-PRODUCT-WRONG-TIME-" + product_code;
                            break;
                    }

                } else {
                    process_mo.setNotificationCode("CHECK-PRODUCT-NOT-EXIST");
                    mo_his_desc = "CHECK-PRODUCT-NOT-EXIST";
                }
